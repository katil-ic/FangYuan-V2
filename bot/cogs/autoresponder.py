...
class Autoresponder(commands.Cog):
    ...
    async def _show_list(self, ctx):
        ...
        embed = discord.Embed(title=f"<:ownerinfo:1523725199457910884> Autoresponders ({len(rules)})", color=0x5865F2)
        lines = []
        for i, r in enumerate(rules, 1):
            status = "<a:tick:1523383850749792397>" if r.get("enabled", True) else "<:Xieron_stolen_emoji_1774597520:1520895245733204039>"
            match_icon = {"exact": "<:ownerinfo:1523725199457910884>", "startswith": "⬅<:ownerinfo:1523725199457910884>", "endswith": "<a:pink_arrow_haveli:1523620310124068985>", "regex": "<:ownerinfo:1523725199457910884>", "contains": "<:ownerinfo:1523725199457910884>"}.get(r.get("match_type", "contains"), "<:ownerinfo:1523725199457910884>")
    ...
    async def ar_info(self, ctx, *, trigger: str):
        ...
        if not rule:
            return await ctx.send(embed=make_embed(f"<:Xieron_stolen_emoji_1774597520:1520895245733204039> No rule for `{trigger}`.", self.bot.error_color))
        embed = discord.Embed(title=f"<:ownerinfo:1523725199457910884> Autoresponder: {rule['trigger']}", color=0x5865F2)
